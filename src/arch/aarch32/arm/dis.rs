    ...
    L: Literal,
{
    ...
    let address_operand = match (immediate_bit, is_preindex, is_wbit, is_shifted) {
        (0, true, true, _) => vec![
            rd_operand,
            op::suff(op::wrap("[", vec![rn_operand, op::lit(offset12)], "]"), "!"),
        ],
        ...
        (0, true, false, _) => vec![
            rd_operand,
            op::wrap("[", vec![rn_operand, op::lit(offset12)], "]"),
        ],
...
fn bl<L>(pc: &memory::Pointer<PtrVal>, cond: u32, l: u32, offset: u32) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
    let is_link = l != 0;
...
fn swi<L>(pc: &memory::Pointer<PtrVal>, cond: u32, offset: u32) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
    ...
    L: Literal,
{
    ...
    let op_list = match immediate_bit {
        1 => vec![xpsr, op::lit(data_immed << (shift_imm & 0xFFFF_FFFE))],
...
fn mrs<L>(cond: u32, r: u32, rd: A32Reg) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
...
fn bx<L>(p: &memory::Pointer<PtrVal>, cond: u32, rm: A32Reg) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
...
fn bxj<L>(cond: u32, rm: A32Reg) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
...
fn clz<L>(cond: u32, rd: A32Reg, rm: A32Reg) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
    Ok(Disasm::new(
...
fn blx_register<L>(p: &memory::Pointer<PtrVal>, cond: u32, rm: A32Reg) -> aarch32::Result<Disasm<L>>
where
    L: Literal,
{
    //BX PC is completely valid! And also dumb.
    ...
    L: Literal,
{
    ...
    let cop1_sym = op::lit(opcode_1);
    let cop2_sym = op::lit(opcode_2);
    ...
    L: Literal,
{
    ...
    let operands = match (is_preindex, is_offsetadd, is_writeback, is_immedoffset) {
        ...
        (true, false, false, true) => vec![
            ...
            op::wrap(
                "[",
                vec![op::sym(&rn.to_string()), op::lit(-(immedoffset as i32))],
                "]",
            ),
        ],
    ...
    L: Literal,
{
    ...
    let operands = match (is_preindex, is_wbit) {
        ...
        (true, false) => vec![
            cp_op,
            crd_op,
            op::wrap("[", vec![rn_operand, op::lit(offset * 4)], "]"),
        ],
        (true, true) => vec![
            cp_op,
            crd_op,
    ...
    L: Literal,
{
    ...
    let operands = match d {
        0 => vec![rd_sym, rn_sym, rm_sym, op::pref("LSL", op::lit(shift_imm))],
        2 => vec![rd_sym, rn_sym, rm_sym, op::pref("ASR", op::lit(shift_imm))],
        _ => return Err(analysis::Error::Misinterpretation(4, false)),
    };
    ...
    L: Literal,
{
    ...
    let address_operand = match (immediate_bit, is_shifted) {
        (0, false) => vec![op::wrap("[", vec![rn_operand, op::lit(offset12)], "]")],
        (1, true) => vec![op::wrap(
            "[",
            vec![
                rn_operand,
