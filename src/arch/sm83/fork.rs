...
fn memlist_op16(regs: Vec<Register>, p: &memory::Pointer<Pointer>, mem: &Bus) -> sm83::Result<(Vec<Register>, Vec<memory::Pointer<Pointer>>, bool)> {
...
fn memlist_hi8(regs: Vec<Register>, p: &memory::Pointer<Pointer>, mem: &Bus) -> sm83::Result<(Vec<Register>, Vec<memory::Pointer<Pointer>>, bool)> {
    if let Some(val) = mem.read_unit(p).into_concrete() {
        return Ok((regs, vec![mem.minimize_context(p.contextualize(0xFF00 | val as Pointer))], true));
    }

    Ok((regs, vec![p.clone()], false))
}

fn memlist_pc8(regs: Vec<Register>, p: &memory::Pointer<Pointer>, mem: &Bus) -> sm83::Result<(Vec<Register>, Vec<memory::Pointer<Pointer>>, bool)> {
...
fn memlist_indir16(regs: Vec<Register>, p: &memory::Pointer<Pointer>, mem: &Bus, state: &State) -> sm83::Result<(Vec<Register>, Vec<memory::Pointer<Pointer>>, bool)> {
    match (regs.get(0), regs.get(1)) {
        (Some(r0), Some(r1)) => match (state.get_register(*r0).into_concrete(), state.get_register(*r1).into_concrete()) {
            (Some(h), Some(l)) => Ok((regs, vec![mem.minimize_context(p.contextualize((h as Pointer) << 8 | l as Pointer))], true)),
            _ => Ok((regs, vec![], false)),
        },
        _ => Ok((regs, vec![], false)),
    }
...
pub fn prereq(p: &memory::Pointer<Pointer>, mem: &Bus, state: &State) -> sm83::Result<(Vec<Register>, Vec<memory::Pointer<Pointer>>, bool)> {
    match mem.read_unit(p).into_concrete() {
                ...
                Some(subop) => {
                    match ALU_TARGET_REGS[(subop & 0x07) as usize].clone() {
                        ...
                        op::Indirect(sym) => match *sym {
                            op::Symbol(ref name) if name == "hl" => Ok((Register::reglist_from_sym("hl"), vec![], true)),
                            _ => Ok((vec![], vec![], false))
                        },
                        _ => Ok((vec![], vec![], false))
                    }
        ...
        Some(op) => {
            ...
            match ((op >> 6) & 0x03, (op >> 5) & 0x01, (op >> 3) & 0x01, op & 0x07) {
                ...
                (0, _, _, 4) if ((op >> 3) & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //inc [hl]
                (0, _, _, 4) => Ok((vec![], vec![], true)), //inc r8
                (0, _, _, 5) if ((op >> 3) & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //dec [hl]
                (0, _, _, 5) => Ok((vec![], vec![], true)), //dec r8
                (0, _, _, 6) if ((op >> 3) & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //ld [hl], d8
                (0, _, _, 6) => Ok((vec![], vec![], true)), //ld r8, d8
                (0, _, _, 7) => Ok((vec![], vec![], true)), //8080 bitwise ops
                (1, _, _, _) if ((op >> 3) & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //ld [hl], r8
                (1, _, _, _) if (op & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //ld r8, [hl]
                (1, _, _, _) => Ok((vec![], vec![], true)), //ld r8, r8
                (2, _, _, _) if (op & 0x07) == 6 => Ok((Register::reglist_from_sym("hl"), vec![], true)), //aluops r8, [hl]
