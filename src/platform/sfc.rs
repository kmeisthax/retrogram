...
impl<D> Image for SufamiPlatform<D>
where
    D: Decoder,
{
...
pub fn construct_platform_from_rom_image<F>(
    file: &mut F,
    pv: PlatformVariant,
) -> io::Result<w65c816::Bus>
where
    F: io::Read + io::Seek,
{
    ...
    let clean_data = data.get(floffset..).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "ROM header could not be stripped or ROM is shorter than 1KB",
        )
    })?;
    ...
    rom_mapping = rom_mapping.known_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            "Could not automatically determine ROM mapping",
        )
    })?;
    m.install_rom_image(
        u24::from(0 as u16),
