...
impl<D> Image for SufamiPlatform<D> where D: Decoder {
    ...
    fn decode_addr(&self, ptr: &Pointer<Self::Pointer>, _base: Self::Pointer) -> Option<Self::Offset> {
...
pub fn construct_platform_from_rom_image<F>(file: &mut F, pv: PlatformVariant) -> io::Result<w65c816::Bus> where F: io::Read + io::Seek {
    ...
    m.install_rom_image(u24::from(0 as u16),
        u24::try_from(0xFFFFFF as u32).or_else(|_| Err(io::Error::new(io::ErrorKind::InvalidData, "ROM size is too large for the SFC platform")))?,
